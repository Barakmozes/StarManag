...
function sortOrdersByBusinessTime<T extends { orderDate?: any; createdAt?: any }>(
  ...
  return [...arr].sort((a, b) => {
    const ta = toDate(a.orderDate)?.getTime() ?? toDate(a.createdAt)?.getTime() ?? 0;
    const tb = toDate(b.orderDate)?.getTime() ?? toDate(b.createdAt)?.getTime() ?? 0;
...
function typeLabel(o: any) {
  ...
  if (o?.tableId) return { label: "שולחן", icon: <HiOutlineBuildingStorefront size={18} /> };
  if (o?.pickupTime) return { label: "איסוף", icon: <HiOutlineClock size={18} /> };
    ...
    return (
      <Container>
        <div className="mt-8 space-y-6">
          ...
          <div className="h-10 w-80 bg-slate-100 rounded-full animate-pulse mx-auto" />
    ...
    return (
      <Container>
        <div className="mt-12 flex flex-col items-center text-center gap-3">
          ...
          <Link
            href="/user"
            className="inline-flex items-center justify-center bg-green-600 text-base px-4 py-2 text-white border border-green-500 space-x-2 rounded-full hover:text-green-700 hover:bg-green-200 transition"
          >
            <HiOutlineChevronLeft />
  ...
  return (
    <Container>
      ...
      <div className="mt-6 text-center space-y-3">
        <h2 className="text-lg md:text-2xl lg:text-3xl tracking-tight text-gray-700 sm:text-4xl">
          ההזמנות שלי
        </h2>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          ...
          {totalCount > 0 && (
            <div className="inline-flex items-center gap-2 rounded-full border bg-white px-3 py-2 shadow-sm">
              <button type="button" onClick={() => setSortDir("desc")} className={badgeBase(sortDir === "desc")}>
                חדש → ישן
              </button>
              <button type="button" onClick={() => setSortDir("asc")} className={badgeBase(sortDir === "asc")}>
                ישן → חדש
              </button>
      ...
      {totalCount < 1 && (
        <div className="mt-12 flex flex-col items-center text-center gap-3">
          ...
          <div className="text-sm text-slate-500">ברגע שתבצע הזמנה — היא תופיע כאן.</div>
        </div>
      )}
        ...
        <section className="py-10">
          <div className="flex items-end justify-between mb-4">
            <h3 className="text-xl font-semibold text-gray-800">
              הזמנות שולחן{" "}
              <span className="text-sm text-slate-500 font-normal">({tableOrders.length})</span>
            </h3>
          </div>
      ...
      {otherOrders.length > 0 && (
        <section className="py-10">
          <div className="flex items-end justify-between mb-4">
            <h3 className="text-xl font-semibold text-gray-800">
              הזמנות{" "}
              <span className="text-sm text-slate-500 font-normal">({otherOrders.length})</span>
            </h3>
          </div>
...
function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <div className="group flex flex-col border rounded-xl bg-white shadow-sm hover:shadow-md transition overflow-hidden">
      {/* Modal trigger area */}
      <div className="p-3">
        {isDelivered ? <UserDeliveredModal order={order} /> : <UserOnDeliveryModal order={order} />}
      ...
      <div className="mt-auto border-t bg-slate-50 p-3">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col items-end gap-2">
            <span className={["text-xs px-2 py-1 rounded-full border font-semibold", statusBadge(o.status)].join(" ")}>
            ...
            <span
              className={[
                ...
                o.paid ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-700 border-slate-200",
              ].join(" ")}
            >
              {o.paid ? "שולם" : "לא שולם"}
            </span>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="inline-flex items-center gap-2 text-xs px-2 py-1 rounded-full border bg-white text-slate-700">
            ...
            {o.preOrder ? <span className="text-slate-500">• PreOrder</span> : null}
          </span>

          {o.discount != null && Number(o.discount) > 0 && (
            <span className="inline-flex items-center gap-2 text-xs px-2 py-1 rounded-full border bg-white text-slate-700">
              הנחה: <span className="font-semibold">{formatMoney(o.discount)}</span>
            </span>
          )}
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <div className="flex items-center gap-2 rounded-lg bg-white border p-2">
            <HiOutlineCalendarDays size={18} className="text-slate-500" />
            <div className="leading-tight">
              <div className="text-[11px] text-slate-500">תאריך הזמנה</div>
              <div className="text-xs font-semibold text-slate-700">{orderedAt}</div>
            </div>
          </div>

          <div className="flex items-center gap-2 rounded-lg bg-white border p-2">
            <HiOutlineClock size={18} className="text-slate-500" />
            <div className="leading-tight">
              <div className="text-[11px] text-slate-500">עודכן</div>
              <div className="text-xs font-semibold text-slate-700">{updatedAt}</div>
            </div>
          </div>
        </div>
        ...
        {(o.deliveryTime || o.pickupTime) && (
          <div className="mt-2 grid grid-cols-2 gap-2">
            <div className="rounded-lg bg-white border p-2">
              ...
              <div className="text-xs font-semibold text-slate-700">{deliveryTime}</div>
            </div>
            <div className="rounded-lg bg-white border p-2">
              <div className="text-[11px] text-slate-500">זמן איסוף</div>
              <div className="text-xs font-semibold text-slate-700">{pickupTime}</div>
            </div>
          </div>
        )}
        ...
        {!o.tableId && !o.pickupTime && o.deliveryAddress && (
          <div className="mt-2 rounded-lg bg-white border p-2">
            <div className="text-[11px] text-slate-500">כתובת משלוח</div>
            <div className="text-xs font-semibold text-slate-700 line-clamp-2">{o.deliveryAddress}</div>
          </div>
        )}
